#!/usr/bin/env bash
# Build the PaideiaOS exception kernel.
# M3-002 (#715): Dedicated build for kernel-exc3.elf that links exception entry point.
#
# Invokes paideia-as on every .pdx file under src/kernel/, links the
# resulting objects via src/kernel/link-exc3.ld using boot_stub_exc3.S,
# produces build/kernel-exc3.elf.
...
LINK_SCRIPT="${KERNEL_SRC}/link-exc3.ld"
...
echo "[boot-stub-exc3] tools/boot_stub_exc3.S -> boot_stub_exc3.o (32+64-bit, as --64)"
BOOT_STUB_OBJ="${BUILD_DIR}/boot_stub_exc3.o"
as --64 -o "${BOOT_STUB_OBJ}" "${KERNEL_SRC}/boot_exc3/boot_stub_exc3.S"
...
while IFS= read -r -d '' pdx; do
    ...
    OBJECTS+=("${obj}")
done < <(find "${KERNEL_SRC}" -name '*.pdx' -not -path '*/boot_panic/*' -print0 | sort -z)
...
echo "[link] ld -T link-exc3.ld -> kernel-exc3.elf"
ld -nostdlib --warn-common --fatal-warnings -T "${LINK_SCRIPT}" -o "${BUILD_DIR}/kernel-exc3.elf" "${OBJECTS[@]}"

echo "[audit] R_X86_64_32 relocations must not target high-VA (>= 0xffff800000000000) symbols"

NM_MAP="${BUILD_DIR}/.nm-exc3.map"
SEC_MAP="${BUILD_DIR}/.sec-exc3.map"

nm build/kernel-exc3.elf | awk 'NF>=3 {print $3, $1}' > "${NM_MAP}"
readelf -SW build/kernel-exc3.elf \
...
echo "[ok] ${BUILD_DIR}/kernel-exc3.elf"
