...
ec_confine_one '_pxt_abort_scratch'         'core/cap/kind_pdxfs_txn.o'
ec_confine_one '_pxt_create_ctx'            'core/cap/kind_pdxfs_txn.o'
...
#
# _kind_bc_table + _kind_bc_stats are the only places in the
...
# kind_block_cache.pdx). Names carry the `_kind_bc_` prefix to stay
# disjoint from block_cache.pdx's own `_block_cache_*` CLOCK-cache
# statics; the two modules own separate authority slices (handle-row
# vs. cache-line arena) that must not share symbols.
R52_KBC_SRC="${REPO_ROOT}/src/kernel/core/cap/kind_block_cache.pdx"
if [[ ! -f "${R52_KBC_SRC}" ]]; then
    echo "[r52-m7-confine] FAIL - kind_block_cache.pdx missing" >&2
    exit 1
fi
ec_confine_one '_kind_bc_table' 'core/cap/kind_block_cache.o'
ec_confine_one '_kind_bc_stats' 'core/cap/kind_block_cache.o'
if [[ "${EC_CONFINE_OK}" != "1" ]]; then
