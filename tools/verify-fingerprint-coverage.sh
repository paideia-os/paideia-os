...
ALLOWLIST = {
    ...
    # scope note.
    "init home mount ok [legacy: INIT HOME MOUNT OK] -- src=/var/pdxfs/home.img backend=PDXFS_BLOCK":
        "R65v2.M1-001 (#1979). R106.M3 (#2230) rewrote the mount-point "
        "field from the retired /home/operator literal to a static "
        "'<fp>' placeholder token; R106.M3-followup (#2439) then "
        "retired the mp= field entirely — it duplicated nothing not "
        "already in src=/backend= and its placeholder shape was a "
        "golden-fragility landmine ahead of R108.M2's real-fingerprint "
        "substitution. Fires only when /var/pdxfs/home.img exists AND "
        "sys_mount succeeds; neither holds under the default "
    ...
    # succeed.
    "init home mount ok [legacy: INIT HOME MOUNT OK] -- src=/dev/nvme0 backend=PDXFS_BLOCK":
        "R65v2.M1-001 (#1979) / R65v2.M1-002 device-target path (#1980). "
        "R106.M3 (#2230) rewrote the mount-point field from the retired "
        "/home/operator literal to a static '<fp>' placeholder token; "
        "R106.M3-followup (#2439) then retired the mp= field entirely "
        "(same rationale as the file-target sibling above). Fires only "
        "when /dev/nvme0 exists AND sys_mount succeeds; no devfs node "
        "ever populates that path in this tree yet, and backend_id=5 is "
        "UNIMPL regardless. Assertable once real hardware/devfs + "
        "R51/R52 pdxfs-block land together.",
