...
| 0x1A9  | `KIND_PACKET_FILTER`     | `KIND_IPC_ENDPOINT = 5`           | R96.M3-001 (paideia-os #2091, ratified 2026-09-01): **reserved, unimplemented.** Future packet-filter-chain installation authority. Chain semantics (numeric priority 0-999, `Drop` short-circuits, `Accept` continues, tie-break by registration order) are already specified at `design/network/filter-chain.md` — do not re-decide them when this ordinal is implemented. Round-boundary ratification recorded at `design/networking/filter-chain.md`. | reserved                       | —                    | —                                |
| 0x1AA  | `KIND_TLS_CONN`          | `KIND_TCP_SOCKET = 0x1AB`         | R97.M2-001 (paideia-os #2095, ratified 2026-09-01): **reserved, unimplemented.** Future in-kernel PQ-hybrid TLS connection, pending paideia-as gaining the classical bridge primitives (HKDF-SHA256 v0.26.0 ✓, SHA-256 v0.25.0 ✓, X25519 v0.27.0 ✓, ECDSA/RSA certificate verification NOT landed — paideia-as#1346 filed R97.M4-001) a real-Internet TLS 1.3 handshake needs. Until then, TLS lives entirely in user-space per `design/network/stack.md` NET-D6, formally ratified at `design/networking/tls-placement-decision.md`. Base ordinal reflects fix#2005's TCP renumber. | reserved                       | —                    | —                                |
...
| 0x1AD  | `KIND_NIC`               | `KIND_DEVICE = 10`                | R91.M1-001 (paideia-os #2011, landed): one NIC's device-level identity + link state (nic_kind selector: 0=NONE, 1=E1000E, 2=VIRTIO, 3=RTL8139; 6-byte MAC packed little-endian into a u64's low 48 bits; link_state: 0=DOWN, 1=UP). Row layout is 16 bytes (header + mac_word); pool holds 4 rows. Derived over `KIND_DEVICE`, root-minted by supervisor/driver-bringup code that already holds a `KIND_DEVICE` parent with `RIGHT_MINT`; the kind half of the derivation gate discriminates NOTHING on its own (every PCI function is a `KIND_DEVICE`), matching the honest division `KIND_BATTERY` / `KIND_THERMAL_ZONE` write down. Immutable-once-minted at this landing: `R_NIC_QUERY` (0x001) is the only right; three arity-one QUERY ops (QUERY_KIND, QUERY_MAC, QUERY_LINK) are the entire `cap_invoke` surface. Later milestones wiring real rx/tx-ring authority or MAC programming do so via SEPARATE derived kinds (same posture `KIND_TB_DOMAIN` + `KIND_TB_ROUTE` take under `KIND_DEVICE`). Reallocated from the earlier R91.M1-001 reservation at `0x1A7` — that placeholder never landed and was superseded by `KIND_TLS_TRUST` (#2007); `0x1AD` is the next free tag past the R91-R99 networking reservation block. Handler folded into the same file as row + mint (`cap_handler_nic` in `kind_nic.pdx`) because the op surface is deliberately tiny. Dispatched via `cap_invoke_dispatch` (`cmp rcx, 0x1AD; je call_kind_nic`). | derived                       | `0xFFFFEB28..0xFFFFEB2F` | `core/cap/kind_nic.pdx`         |
| 0x1AE  | `KIND_DISPLAY_BACKEND`   | `KIND_DEVICE = 10`                | R101.M1-001 (paideia-os #2136, landed): the multi-backend anchor above the Iris-Xe-specific `KIND_DISPLAY_ENGINE` (0x16F). `KIND_DISPLAY_ENGINE`'s mint gate refuses `gpu_gen ∉ {1,2,3}` so a QEMU stdvga or virtio-gpu cannot mint one; this kind sits above it (design/graphics/r101-kernel-plan.md §4.1 Option C) and carries a `backend_kind` selector distinguishing `BOCHS_STDVGA=1`, `VIRTIO_GPU_2D=2`, `VIRTIO_GPU_3D=3`, `IRIS_XE=4`. Row layout is 32 bytes (header + bar_pa + mmio_base + provider_slot); pool holds 4 rows. Derived over `KIND_DEVICE`, root-minted by driver-bringup code that already holds a `KIND_DEVICE` parent with `RIGHT_MINT`; the kind half of the derivation gate discriminates NOTHING on its own (every PCI function is a `KIND_DEVICE`), matching the honest division `KIND_NIC` / `KIND_BATTERY` write down. Immutable-once-minted at this landing: `R_DPYB_QUERY` (0x001) is the only right; one arity-one QUERY op (QUERY_KIND) is the entire `cap_invoke` surface. `R104.M4` introduces `KIND_PAGE_FLIP` as a SEPARATE derived kind under `KIND_DISPLAY_OUTPUT` for the compositor's per-output swap authority -- widening this kind's rights mask would let a holder of pure backend authority reach the per-output flip path, inverting the layering §4.3 encodes. Handler folded into `kind_display_backend.pdx` because the op surface is deliberately tiny at R101. First successful mint publishes `_active_backend_kind` (consumed by `core/graphics/backend_dispatch.pdx`'s `backend_flush` dispatch and by `core/graphics/sim_vblank.pdx`'s LAPIC-tick vblank gate). | derived                       | `0xFFFFEB30..0xFFFFEB3F` | `core/cap/kind_display_backend.pdx` |
| 0x1AF  | `KIND_FRAMEBUFFER`       | `KIND_MEMORY = 4`                 | R101.M3-001 (paideia-os #2145, landed): the GPU-optional scan-out kind that lets Bochs/stdvga (no GPU) participate in the scan-out authority chain. `KIND_GPU_BO` (0x174) presupposes a `KIND_GPU_VM` (0x175) parent which does NOT exist on Bochs/stdvga; a stub `KIND_GPU_VM` promising nothing would let a caller derive scan-out buffers from an empty authority, a design smell §4.2 explicitly rejects. This kind is the GPU-optional alternative: a linear `KIND_MEMORY` range with pixel geometry (`width`/`height`/`stride`/`pixel_format`) and a `backend_kind` selector (packed into the header from the parent `KIND_DISPLAY_BACKEND` row) so the flush dispatch stays coherent. Row layout is 48 bytes (header + width×height packed + stride + lfb_pa + lfb_va + owner_pid); pool holds 4 rows. Derived over `KIND_MEMORY`, gated on `RIGHT_MINT` per the R89 `KIND_TUI_CANVAS` precedent. Rights: `R_FB_MAP` (0x001), `R_FB_FLIP` (0x002), `R_FB_REVOKE` (0x004). Handler folded (op surface at R101 is QUERY-family only; `sys_framebuffer_create` / `sys_page_flip` arrive at R105). Format enum: `BGRA8888=1`. Dispatched via `cap_invoke_dispatch` (`cmp rcx, 0x1AF; je call_kind_framebuffer`). See design/graphics/r101-kernel-plan.md §4.2. | derived                       | `0xFFFFEB40..0xFFFFEB4F` | `core/cap/kind_framebuffer.pdx` |
...
- **Next free derived-kind tag:** `0x1B5` (after R101.M1-001 + R101.M3-001 landed `KIND_DISPLAY_BACKEND` at `0x1AE` and `KIND_FRAMEBUFFER` at `0x1AF`; `0x1B0` + `0x1B1` are `KIND_PAGE_FLIP` (R104.M4) and `KIND_HOTPLUG_CHANNEL` (R105.M4) per design/graphics/r101-kernel-plan.md §4.3 / §5; `0x1B2` is `KIND_SCHEMA_HANDLE` (R90-XREPO.012.M3-001, paideia-os #2126); `0x1B3` is `KIND_VOLUME_SNAPSHOT` and `0x1B4` is `KIND_KEK` (R90-XREPO.LV11.M3-001 / M5-001, paideia-os #2223 / #2224) -- next truly free tag past the R101/R103/R104/R105 GUI block and the R90-XREPO.LV11 wave is `0x1B5`). The vacated `0x1A4` slot is **not** treated as free — do not reuse until a dedicated audit confirms no stale reference remains; `0x1A5` continues to name `KIND_PDXFS_MOUNT_TABLE`. **`0x197`..`0x1A3` are unaudited** by any pass of this file — treat that range as "unknown, not confirmed free" until a dedicated audit walks `core/cap/kind.pdx` end to end; the R72/R89 backfill above only proves this file had silently fallen behind, not that `0x197..0x1A3` is empty.
