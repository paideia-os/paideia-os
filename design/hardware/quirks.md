...
| Subsystem | Quirk | Impact | Handling | Status | Round observed | Source |
|-----------|-------|--------|----------|--------|----------------|--------|
| VMD | Intel VMD Controller enabled by default; NVMe appears behind VMD as a `pci-domain`-style controller rather than bare PCIe. R22 PCI enumerator (bus-0 recursive descent) cannot see the drive as a first-class endpoint while VMD is on — VMD hides its children behind a proprietary indirection. **RESTRICTION LIFTED AT R47** (issues #1412–#1426). The R47 VMD driver substrate (probe + endpoint enumeration + virtual root-complex config-space aperture + MSI-X remap + hotplug + NVMe-over-VMD bridge + per-endpoint IOMMU domain) makes children reachable through KIND_VMD_ENDPOINT (0x187) with the BIOS toggle left in its factory-default **Enabled** position; the BIOS-off recipe below remains supported as a fallback but is no longer required. | Pre-R47: R24 NVMe bring-up could not see the drive with VMD enabled. R47 sources: `src/kernel/core/drivers/vmd/{probe,endpoint_enum,root_complex,msix_remap,hotplug,nvme_bridge,nvme_isolation,iommu_domain,iommu_switch,bios_off_fallback}.pdx`, `src/kernel/core/cap/kind_vmd_endpoint.pdx`, `src/kernel/core/ipc/vmd_config_channel.pdx`. Structural witnesses at `tests/kernel/{drivers/vmd,cap,ipc}/`; HW-smoke recipe at `tools/hw-smoke-r47-vmd.md`. **Fallback for operators who prefer to leave VMD Disabled:** the pre-R47 recipe (BIOS Setup → Storage → Intel VMD Controller → Disabled before any R22 PCI enum / R23 driver-plane / R24 NVMe boot) still works and is exercised by `tests/kernel/drivers/vmd/bios_off_fallback_synth.pdx` (R47.M5-001 #1424). Full operator recipe with BIOS toggle path + GDB attach + witness-invocation ceremony at `tools/nvme-hw-smoke.md` §1.2.1 (R24.M6-001 #908). Post-VMD-off verification: `pci_enumerate_all(0)` records an NVMe controller at approximately `00:0E.0` instead of the VMD RAID container, and `nvme_probe` fingerprint reads `NVME PROBE N=1`. | PROVISIONAL | — | Lenovo Insyde BIOS default (T14 G4). `design/roadmap/r18-plus-bare-metal.md` §7 + `design/roadmap/r19-t14-g4-boot-guide.md` §3. Elevated in scope at R22.M6-001 (#870) to add the R22 PCI-enumerator impact + explicit BIOS toggle path + first-light verification recipe. Re-elevated at R24.M6-001 (#908) with a `SEVERITY: LOAD-BEARING` note + cross-reference to `tools/nvme-hw-smoke.md §1.2.1` (the operator recipe) + `tests/kernel/drivers/nvme/hw_smoke.pdx` (the witness that verifies the toggle worked). **Restriction lifted at R47 close (#1412–#1426, R47.M5-002 #1425)** — the VMD driver substrate lands the alternative path so operators need not toggle BIOS; the row stays PROVISIONAL until the R47 HW-smoke on the T14 G4 promotes it to `WORKED-AROUND` per `tools/hw-smoke-r47-vmd.md`. |
...
*Updated 2026-08-21 at R47 close (#1412–#1426; documentation issue
R47.M5-002 #1425) — §2.4 VMD row's "Quirk" and "Handling" columns
enriched with the R47 driver-substrate landing.  The BIOS-off
recipe stays in the row as a fallback (still exercised by
`tests/kernel/drivers/vmd/bios_off_fallback_synth.pdx`) but is no
longer required.  Operators may leave the Intel VMD Controller
BIOS setting in its factory-default Enabled position and reach
child endpoints through the R47 driver stack (KIND_VMD_ENDPOINT
0x187 + virtual root-complex + NVMe-over-VMD bridge + per-endpoint
IOMMU domain).  Row stays `PROVISIONAL` until `tools/hw-smoke-r47-
vmd.md` promotes it to `WORKED-AROUND` on the T14 G4.  No new rows
added at this pass.*
