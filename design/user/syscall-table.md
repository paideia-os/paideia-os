...
| 115 | `semantic_send` | `schema`, `record_ptr`, `record_len` | 0 or `-errno`. Kernel-side substrate for the libpdx-semantic-pipe producer stub postui#42 currently short-circuits to `xor rax, rax; ret`. Copies `record_len` bytes from `record_ptr` (via `user_ptr_ok` + `user_read_bytes_via_walk`) into the next slot of a fixed 16-slot global ring (`_semantic_ring`, 4 KiB total, 256-byte stride: `schema u64` @+0, `len u64` @+8, `data [u8;240]` @+16); commits the header only after a successful walk-copy so a mid-copy `-EFAULT` leaves the previous slot header intact. Advances `_semantic_ring_head` modulo 16 on success -- overwrite-oldest ring policy (`PIPE_FULL` never fires at v1). `-EINVAL` on `record_len == 0` or `record_len > 240`; `-EFAULT` on bad `record_ptr` / walker fault. Schema is opaque to the kernel; caller-side schema whitelisting lives in libpdx-semantic-pipe. This is the PRODUCER-only landing -- the consumer (either `sys_semantic_recv` or a KIND_SEMANTIC_PIPE cap-invoke op) arrives in a follow-on round; the ring layout is stable so recv is additive (adds a tail cursor + wake-list without repacking). Sysno chosen as the first slot above `sys_volume_mint` (114); bounds check widened from 114 to 115 in the same edit. R107-M0-001 (paideia-os #2350). |
| 116 | `sched_wait_ns` | `ns` | always 0. Puts the calling task into `TASK_STATE_WAITING` for at least `ns` nanoseconds via a real timer-wheel entry (`SchedWaitNs.sched_wait_ns`, `sched/sched_wait_ns.pdx`) rather than a ring-3 busy-wait re-reading `sys_clock_read_ns` -- the pattern svc-compositor's `render_loop` used before this landing. Records `(SCHED_WAIT_TIMER_NS=3, 0, deadline=hpet_now_ns()+ns)` into the caller's TCB (`+448`/`+456`/new field `+480`) and calls `sched_block`; `sched_wait_ns_sweep`, wired into `int/exceptions.pdx handle_timer`'s per-tick path (every ~10 ms LAPIC tick, not gated like the TCP retransmit sweep), scans for expired deadlines and calls `sched_wake`. Wake latency is therefore bounded by one tick (~10 ms) past the requested `ns`, not exact -- acceptable for render pacing, not a hard-real-time guarantee. `ns == 0` returns immediately without blocking. `SCHED_WAIT_TIMER_NS` is 3, not 2, because `KindPageFlip`'s `SCHED_WAIT_PAGE_FLIP` (sysno 112's own wait key) already claims 2 against the same TCB fields. No user pointers -- `ns` is a plain duration, not a VA. COMP-IMPL-07 (Wave UUU, paideia-os compositor-substrate backtrack). |
| 122 | `openpty` | `rows`, `cols` | `master_fd \| (slave_fd << 8) \| (cap_slot << 16)` or `-errno`. Mints a KIND_PTY pair (`pty_cap_mint`, `kind_pty.pdx`) then allocates two fd_table slots (`fd_alloc`/`fd_set` x2) tagged with `kind_pty.pdx`'s PTY_FD_TAG (bit 15) instead of a vfs `vnode_idx`; wires both fd numbers back into the row (`pty_row_set_fds`). `-EINVAL` if `pty_cap_mint` refused (bad rows/cols in `[1,4096]`, row-pool or cap_table exhaustion -- all three collapse to one sentinel at the kind layer). `-EMFILE` if either `fd_alloc` has no free slot; on the slave-side failure the master's fd slot, the minted cap, and the pty row are all unwound before returning so no partial allocation leaks. Wave iota ι-02 (paideia-os pdxterm real-body wave). |
| 123 | `grantpt` | `master_fd` | 0 or `-EBADF`. Structural validity check only -- this kernel has no `/dev/pts` and no per-fd ownership model to grant, so `grantpt` confirms `master_fd` is a live PTY-tagged fd on the master side and no-ops otherwise (see `sys_grantpt.pdx`'s header for the full rationale). Wave iota ι-02. |
| 124 | `unlockpt` | `master_fd` | 0 or `-EBADF`. Byte-identical structural-validity posture to `grantpt` (123) -- `sys_openpty` hands back an already-open slave fd directly, so there is no lock to remove. Wave iota ι-02. |
...
- R90-XREPO.LV11 (paideia-os #2223 / #2224 / #2225) — the libpdx-
  ...
  landing details.
- R107-M0-001 (paideia-os #2350) — kernel-side substrate for
  libpdx-semantic-pipe. `sys_semantic_send` (sysno 115) as the first
  slot above `sys_volume_mint` (114); bounds check widened from 114
  to 115 in the same edit. Producer-only landing: 16-slot / 4 KiB
  global ring (`_semantic_ring`, 256-byte stride, schema+len header
  then 240-byte data area). Overwrite-oldest policy. Consumer
  (`sys_semantic_recv` or a KIND_SEMANTIC_PIPE cap-invoke op)
  deferred; the ring layout is intentionally stable so recv is
  additive. Unblocks postui#42 (`SEMANTIC_SEND_LIVE` flip 0->1 and
  stub body swap to `mov rax, 115; syscall; ret`).
