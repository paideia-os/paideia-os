...
| Milestone | Title | Task brief | Acceptance | Issues | Blocked-by | Repo |
|---|---|---|---|---|---|---|
| R121.M1 | KIND_PIPELINE_EDGE capability | Introduce a new kind representing one half of a session-typed pipeline channel. Row layout: 32 bytes = {schema_id u32, edge_id u32, endpoint_role u8 [0=prod, 1=cons], rights u16, reserved u8, buddy_edge_id u32, session_pos u64, in_use_hdr u64}. Base derives from KIND_MEMORY. **LANDED 2026-09-23 as KIND_PIPELINE_EDGE = 0x1F0** (0x1B3..0x1BF strip filled between plan-write and landing; picked first fresh 16-slot page 0x1F0..0x1FF so M2..M6 have an uncontended strip). | Fingerprint `pipe edge mint ok kind=0x00000000000001f0 schema_id=<hex> role=0x0..0<0|1>` fires on every `pipeline_edge_alloc` call under witness (klog emits numeric role, not `prod|cons` symbolic — a follow-up klog wrapper could add symbolic-string variant). | 4 | (none in R121) | paideia-os |
