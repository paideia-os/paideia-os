...
| `KIND_DMA_DOMAIN` (**landed** R29.M5-001/002, #1036/#1037; renamed from `KIND_HW_DMA_DOMAIN` and re-based over `KIND_MEMORY` = slot 4, **not** `KIND_HW` — a DMA domain is a memory-access scope, so its mint gate requires memory authority) | R29.M5-001 (#1036) | tag `0x142`; row-indirection tail `{iommu_ctx_id:u32, bus_dev_fn:u16, capacity_bytes:u64, coherency:u8, parent_slot:u8}` in `_dma_domain_table` | `R_DMA_ALL = 0x618` = `RIGHT_INVOKE` (query/flush) + `RIGHT_REVOKE` + `RIGHT_MINT` (map/unmap page) + `RIGHT_OBSERVE` (fault log / debug print) |
